...
func main() {
	...
	// Start http server
	http.ListenAndServe(env.Getenv("HYDRA_CLIENT_HOST", "http://localhost")+":4445", n)
...
func handleHome(w http.ResponseWriter, _ *http.Request) {
	var authUrl = client.OAuth2Config(env.Getenv("HYDRA_CLIENT_HOST", "http://localhost")+":4445/callback", "offline", "openid").AuthCodeURL(state) + "&nonce=" + state
...
func handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		http.Redirect(w, r, env.Getenv("HYDRA_CLIENT_HOST", "http://localhost")+":4445/consent?challenge="+challenge, http.StatusFound)
		return
	}
...
func handleCallback(w http.ResponseWriter, r *http.Request) {
	...
	token, err := client.OAuth2Config(env.Getenv("HYDRA_CLIENT_HOST", "http://localhost")+":4445/callback").Exchange(context.Background(), r.URL.Query().Get("code"))
	if err != nil {
