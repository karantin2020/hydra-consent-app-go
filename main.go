...
func main() {
	...
	// Start http server
	err = http.ListenAndServe(env.Getenv("HYDRA_CLIENT_HOST", "localhost")+":4445", n)
	if err != nil {
		log.Fatal(err)
	}
	log.Println("Listening on "+env.Getenv("HYDRA_CLIENT_HOST", "localhost")+":4445")
}

// handles request at /home - a small page that let's you know what you can do in this app. Usually the first.
// page a user sees.
func handleHome(w http.ResponseWriter, _ *http.Request) {
	var authUrl = client.OAuth2Config("http://"+env.Getenv("HYDRA_CLIENT_HOST", "localhost")+":4445/callback", "offline", "openid").AuthCodeURL(state) + "&nonce=" + state
...
func handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		http.Redirect(w, r, "http://"+env.Getenv("HYDRA_CLIENT_HOST", "localhost")+":4445/consent?challenge="+challenge, http.StatusFound)
		return
	}
...
func handleCallback(w http.ResponseWriter, r *http.Request) {
	...
	token, err := client.OAuth2Config("http://"+env.Getenv("HYDRA_CLIENT_HOST", "localhost")+":4445/callback").Exchange(context.Background(), r.URL.Query().Get("code"))
	if err != nil {
